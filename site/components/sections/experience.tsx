    ...
    <section id="experience" className="section bg-[var(--surface)] border-y border-[var(--border)]">
      <div className="container-x">
        ...
        <ol className="relative border-l border-[var(--border)] pl-6 md:pl-8 space-y-10">
          {experience.map((job) => {
            const isFreelance = job.track === "freelance";
            return (
              <li key={`${job.company}-${job.role}`} className="relative">
                <span
                  className={cn(
                    "absolute -left-[33px] md:-left-[41px] top-1.5 h-3 w-3 rounded-full border-2 border-[var(--bg)]",
                    isFreelance
                ...
                <div className="flex items-baseline justify-between gap-4 flex-wrap mb-1">
                  ...
                  <span className="font-mono text-xs text-[var(--muted)] whitespace-nowrap inline-flex items-center gap-2">
